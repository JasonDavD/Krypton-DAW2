 ...
 * Portado del monolito; ADAPTACION microservicios: usa el SNAPSHOT del nombre
 * ({@code item.getProductName()}) en vez de la entidad Product (que vive en catalog),
 * así el comprobante es autocontenido y NO necesita Feign. Lee la entidad Orden
 * directamente (server-side, no es contrato de API).
 */
@Component
public class ComprobanteExporter {
    ...
    public byte[] export(Orden order, List<ItemOrden> items) {
        ...
        return render(doc -> {
            ...
            for (ItemOrden it : items) {
                ...
                table.addCell(it.getProductName());   // snapshot del nombre (no llama a catalog)
            ...
            // El micro sí guarda el descuento del cupón: lo mostramos para que el total cuadre.
            if (order.getDiscount() != null && order.getDiscount().signum() > 0) {
                doc.add(totalLine("Descuento (S/): -" + order.getDiscount().toPlainString(), false));
            }
    ...
    // ─── private helpers (mismo estilo que el monolito) ───────────────────────────
